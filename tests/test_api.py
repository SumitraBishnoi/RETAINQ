...
@pytest.mark.asyncio
async def test_copilot_run(client, sample_customer_id):
    r = await client.post("/copilot/run", json={"customer_id": sample_customer_id})
    assert r.status_code == 200
    data = r.json()
    assert data["customer_id"] == sample_customer_id
    assert 0 <= data["churn_probability"] <= 1
...
@pytest.mark.asyncio
async def test_copilot_run_404(client):
    r = await client.post("/copilot/run", json={"customer_id": "NONEXISTENT_999"})
