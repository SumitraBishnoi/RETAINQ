...
from src.agents.orchestrator import run as run_copilot, run_async as run_copilot_async
...
app = FastAPI(
    title="RETAINIQ - AI Customer Retention",
    version="0.1.0",
    description="Churn prediction + SHAP + LLM/RAG + LangGraph multi-agent retention intelligence.",
    lifespan=lifespan,
)
...
async def generate_strategy(req: StrategyRequest) -> StrategyResponse:
    state = await run_copilot_async(req.customer_id)
    if state.get("errors"):
...
async def copilot_run(req: CustomerRequest) -> CopilotResponse:
