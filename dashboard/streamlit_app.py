"""RETAINIQ - AI Customer Retention dashboard built on native Streamlit components."""
...
with hero_l:
    st.markdown(
        "**:violet[LIVE]**  ·  RETAINIQ — AI Customer Retention",
        help="AI Customer Retention — multi-agent churn prediction and retention actions.",
    )
    st.title("Predict churn. Explain it. Save it.")
    st.markdown(
        f"<p style='color:{MUTED}; font-size:1.02rem; max-width:680px; margin-top:-0.4rem;'>"
        "RETAINIQ scores churn risk, surfaces the SHAP-driven "
...
tab_portfolio, tab_customer, tab_kb = st.tabs(
    ["📈  Portfolio", "🧬  Customer Analysis", "📚  Knowledge Base"]
)
    ...
    if run_btn:
        ...
        with st.container(border=True):
            st.markdown(
                ...
                f"<div style='font-size:1.1rem; font-weight:600; color:{TEXT}; margin-top:0.4rem;'>Ready to run RETAINIQ</div>"
                f"<div style='color:{MUTED}; margin-top:0.4rem;'>Pick a customer above and click <b>Run RETAINIQ</b>. "
